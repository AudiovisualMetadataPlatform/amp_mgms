...
def main():
    ...
    annotations = Annotations(args.annotation_in, args.input_video, 
                              'scene_detection', '1.0', vars(args))

    # Get a list of scenes as tuples (start, end) 
    shots = find_shots(args.input_video, '/dev/null', args.threshold)
    ...
    if args.annotation_out:
        for s in shots:
            start = get_seconds_from_timecode(s[0].get_timecode())
            end = get_seconds_from_timecode(s[1].get_timecode())
            annotations.add(start, end, 'scene_detection', {'start_frame': s[0].get_frames(), 'end_frame': s[1].get_frames()})
